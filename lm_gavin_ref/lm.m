...
%p = p(:); y_dat = y_dat(:); t = t(:);  % make column vectors
 ...
 DoF    = Npnt - Npar;                  % statistical degrees of freedom
...
%if ( tensor_parameter && prnt == 3 ) prnt = 2; end
 ...
 if ( var(weight) == 0 )                 % identical weights vector, len(weight)==1 in python
...
%JtWdy  = zeros(Npar,1);
 ...
 if ( ~rem(iteration,2*Npar) || dX2 > 0 )       % iteration / 2*Npar의 나머지가 0일 경우 or dX2 > 0 일 경우 (dX2가 뭐냐)
